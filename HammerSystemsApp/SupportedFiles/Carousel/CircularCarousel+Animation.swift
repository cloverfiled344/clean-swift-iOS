...
extension CircularCarousel {
    ...
    internal func decelerationDistance() -> CGFloat {
        let acceleration: CGFloat = -startVelocity *
            CircularCarouselConstants.decelerationMultiplier *
            (1.0 - decelerationRate)
    ...
    internal func startDecelerating() {
        ...
        if !wrapEnabled {
            if bounceEnabled {
                endOffset = max(-CircularCarouselConstants.bounceDist,
                                min(CGFloat(numberOfItems) - 1.0 + CircularCarouselConstants.bounceDist, endOffset))
            } else {
    ...
    @objc internal func step() {
        ...
        if scrolling && !dragging {
            ...
            pscrollOffset = startOffset + (endOffset - startOffset) * delta
            didScroll()
            ...
            pscrollOffset = startOffset + distance
            didScroll()
            ...
            if abs(time - CGFloat(scrollDuration)) < CircularCarouselConstants.floatErrorMargin {
                ...
                if abs(scrollOffset - clampedOffset(scrollOffset)) > CircularCarouselConstants.floatErrorMargin {
                    ...
                    toggleTime = currentTime
                        - Double(CircularCarouselConstants.maxToggleDuration)
                        * Double(abs(difference))
            ...
            var toggleDuration: TimeInterval = startVelocity != 0.0 ?
                TimeInterval(min(1.0, max(0.0, 1.0 / abs(startVelocity)))) :
                1.0
            toggleDuration = CircularCarouselConstants.minToggleDuration
                + (CircularCarouselConstants.maxToggleDuration
                    - CircularCarouselConstants.minToggleDuration)
                * toggleDuration
    ...
    @objc internal func didScroll() {
        if wrapEnabled || !bounceEnabled {
            ...
            pscrollOffset = clampedOffset(pscrollOffset)
        } else {
            ...
            if scrollOffset < minVal {
                pscrollOffset = minVal
                startVelocity = 0.0
            } else if scrollOffset > maxVal {
                pscrollOffset = maxVal
                startVelocity = 0.0
            }
        }
