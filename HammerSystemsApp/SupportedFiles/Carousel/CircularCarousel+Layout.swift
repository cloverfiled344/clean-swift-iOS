...
extension CircularCarousel {
    ...
    internal func value<T>(forOption option: CircularCarouselOption, withDefaultValue defaultValue: T) -> T {
        return pdelegate?.carousel(self, valueForOption: option, withDefaultValue: defaultValue) ?? defaultValue
    }
    ...
    internal func transformForItemView(withOffset offset: CGFloat) -> CATransform3D {
        ...
        let scaleMultiplier = value(forOption: .scaleMultiplier,
                                    withDefaultValue: CircularCarouselConstants.defaultScaleMultiplier)
    ...
    @objc internal func depthSortViews() {
        let views = itemViews.values.sorted { (aValue, bValue) -> Bool in
            return compare(viewDepth: aValue, withView: bValue)
        }
    ...
    internal func transformItemViews() {
        for index in itemViews.keys {
            transform(itemView: itemViews[index] ?? .init(), atIndex: index)
        }
    }
