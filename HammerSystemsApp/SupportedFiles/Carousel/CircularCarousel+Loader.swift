...
extension CircularCarousel {
    @discardableResult internal func loadView(atIndex index: Int, withContainerView containerView: UIView?) -> UIView? {
        ...
        let view = dataSource.carousel(self,
                                       viewForItemAt: IndexPath(item: index, section: 0),
                                       reuseView: dequeItemView())
    ...
    internal func loadUnloadViews() {
        ...
        for iterator in 0..<numberOfVisibleItems {
            var index: Int = iterator + intOffset
            if wrapEnabled {
