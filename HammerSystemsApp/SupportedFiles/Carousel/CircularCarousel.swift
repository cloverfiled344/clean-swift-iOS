...
@IBDesignable open class CircularCarousel: UIView {
    ...
    weak var pdelegate: CircularCarouselDelegate?
    public var delegate: CircularCarouselDelegate? {
        ...
        set {
            pdelegate = newValue
            if pdelegate != nil && pdataSource != nil {
                setNeedsLayout()
            }
        }
    }

    internal var pdataSource: CircularCarouselDataSource?
    public var dataSource: CircularCarouselDataSource? {
        get {
            return pdataSource
        }
        set {
            pdataSource = newValue
            if pdataSource != nil {
                reloadData()
            }
        }
	...
	var scrollDurationDefault = CircularCarouselConstants.scrollDuration
	var scrollTransitionTimingFunc = CAMediaTimingFunctionName.easeInEaseOut
    ...
    internal var pscrollOffset: CGFloat = 0.0
    var scrollOffset: CGFloat {
        get {
            ...
            if abs(pscrollOffset - newValue) > 0.0 {
                pscrollOffset = newValue
                depthSortViews()
                didScroll()
            }
        }
    ...
    internal var pcontentOffset: CGSize = CGSize.zero
    var contentOffset: CGSize {
        get {
            return pcontentOffset
        }
        set {
            if pcontentOffset != newValue {
                pcontentOffset = newValue
                layoutItemViews()
            }
        }
    }
    ...
    internal var itemViews: [Int: UIView] = [Int: UIView]()
    ...
    func itemView(atPoint point: CGPoint) -> UIView? {
        ...
        let views = itemViews.values.sorted { (aVal, bVal) -> Bool in
            return compare(viewDepth: aVal, withView: bVal)
        }
    ...
    internal func compare(viewDepth viewA: UIView, withView viewB: UIView) -> Bool {
        ...
        let zAVal = transformA.m13 + transformA.m23 + transformA.m33 + transformA.m43
        let zBVal = transformB.m13 + transformB.m23 + transformB.m33 + transformB.m43

        var diff = zAVal - zBVal

        if diff == 0.0 {
            let transformCurItem = currentItemView.superview!.layer.transform

            let xAval = transformA.m11 + transformA.m21 + transformA.m31 + transformA.m41
            let xBval = transformB.m11 + transformB.m21 + transformB.m31 + transformB.m41
            let xCurItem = transformCurItem.m11 + transformCurItem.m21 + transformCurItem.m31 + transformCurItem.m41

            diff = abs(xBval - xCurItem) - abs(xAval - xBval)
        }
